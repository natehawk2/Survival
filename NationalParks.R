...
##### Creating the default color palette in ggplot2 (only for discrete stuff)
# 1. Specify a string of colors you want. Here are some cool ways to do it:
my_gg_colors <- c('blue', 'cornflowerblue', 'dodgerblue', 'navy') # Specify the colors manually
my_gg_colors <- RColorBrewer::brewer.pal(6, 'Dark2') # Use the RColorBrewer package to give
# you some colors (see the documentation for brewer.pal for more help or do
# RColorBrewer::display.brewer.all() to see all the possible palettes)
my_gg_colors <- park_palettes$Arches # Use palette schemes built by the
# R community. More on these color schemes here:
# https://github.com/katiejolly/nationalparkcolors
# 2. Set the string of color codes the global default option for ggplot2
options(ggplot2.discrete.colour = my_gg_colors, # To specify the default 'color' aesthetic
        ggplot2.discrete.fill = my_gg_colors) # To specify the default 'fill' aesthetic
# No need to specify the colors using the scale_color_manual() function in ggplot
##### Extra tip:
# The scales package has a helpful function to visualize what colors you're using in your plots
scales::show_col(my_gg_colors)
# This can be helpful for selecting what colors you want from possible color palettes as
# the first colors in some palettes (especially from the national parks) aren't super visible
# You could visualize them with something like this:
my_gg_colors <- park_palettes$Arches
scales::show_col(my_gg_colors)
# Then select the ones you want for your plots and set the global theme
my_gg_colors <- park_palettes$MtRainier[c(1, 2, 4)]
